...
if ($data && $horaI && $horaT) {
    ...
    if (!$stmt) {
        echo json_encode(["erro" => "Erro na query"]);
        exit;
    }
    ...
    echo json_encode([
        "ocupado" => $result && $result->num_rows > 0
    ]);
    exit;
}
...
if ($data) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $inicio = strtotime($row["horaI"]);
        $fim = strtotime($row["horaT"]);

        if ($inicio === false || $fim === false) continue;
    ...
    echo json_encode(array_values(array_unique($horarios)));
    exit;
}
